...
class Conversation:
    ...
    def __init__(
        ...
        *,
        ...
        self._speaking_timer: threading.Timer | None = None  # 保存最近的 timer
        ...
        with connect(ws_url, max_size=16 * 1024 * 1024) as ws:
            ...
            def input_callback(audio):
                if self._assistant_speaking:
                    return     # Don't send audio if the assistant is speaking
    ...
    def set_assistant_speaking_false(self):
            self._assistant_speaking = False
            print('Start Speaking Again.')
            # 计时器已经完成，清空引用
            self._speaking_timer = None
            
    # 外部调用的接口：重新“静音”一段时间
    def wait_assistant_speaking(self, delay: float = 5.0):
        """
        如果已经有旧的计时器在等待，则先取消它，
        以新建立的计时器为准。
        """
        print('Stop Speaking for {} seconds.'.format(delay))
        ...
        # 2. 创建新计时器
        timer = threading.Timer(delay, self.set_assistant_speaking_false)
        timer.daemon = True
        timer.start()

        # 3. 记录下来，供下次取消
        self._speaking_timer = timer

    def _handle_message(self, message, ws):
        
        # print(message["type"])
        
        if message["type"] in ("audio", "agent_response"):
        ...
        if message["type"] == "conversation_initiation_metadata":
            ...
            self.wait_assistant_speaking(audio_time)  # Call the function to set assistant speaking to True
